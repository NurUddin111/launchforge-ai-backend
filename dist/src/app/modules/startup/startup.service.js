...
const getMyStartups = async (userId, query) => {
...
const softDeleteStartup = async (id, userId) => {
    const startup = await prisma.startup.findFirst({
        where: {
            id,
            userId,
...
const exportStartup = async (id, userId) => {
    const startup = await prisma.startup.findFirst({
        where: {
            id,
            userId,
    ...
    const content = startup.generatedContent;
...
${(content.mvpFeatures ?? []).map((item) => `- ${item}`).join("\n")}

---

## Tech Stack

${(content.techStack ?? []).map((item) => `- ${item}`).join("\n")}
